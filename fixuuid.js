...
fs.writeFileSync('app/index.tsx', [
  ...
  "    const { data, error } = await supabase.from('profiles').select('*').eq('uuid', userId).single();",
  ...
  "      .from('profiles')",
  "      .upsert({ uuid: user.id, name, role, location, budget, lifestyle, smoking, pets, bio })",
  "      .select()",
  "      .single();",
...
// Fix TabNavigator to pass uuid correctly
fs.writeFileSync('src/navigation/TabNavigator.js', [
  ...
  "    setMatches(prev => prev.find(m => m.uuid === profile.uuid) ? prev : [profile, ...prev]);",
  "  };",
  "",
  "  const renderScreen = () => {",
  "    switch (activeTab) {",
...
// Fix DiscoverScreen to pass uuid correctly
fs.writeFileSync('src/screens/DiscoverScreen.js', [
  ...
  "  const { profiles, loading } = useProfiles(userProfile?.uuid);",
