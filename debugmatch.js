...
fs.writeFileSync('src/navigation/TabNavigator.js', [
  ...
  "    console.log('Loading matches for:', userProfile.id);",
  "    const { data, error } = await supabase",
  "      .from('matches')",
  "      .select('matched_user_id, profiles!matches_matched_user_id_fkey(*)')",
  "      .eq('user_id', userProfile.id);",
  "    console.log('Matches loaded:', data, 'Error:', error);",
  "    if (error) return;",
  ...
  "    console.log('handleMatch called with:', profile?.name, profile?.id);",
  "    if (matches.find(m => m.id === profile.id)) {",
  "      console.log('Already matched, skipping');",
  "      return;",
  "    }",
  "    const { data, error } = await supabase",
  "      .from('matches')",
  ...
  "      .select();",
  "    console.log('Match saved:', data, 'Error:', error);",
  "    if (error) return;",
